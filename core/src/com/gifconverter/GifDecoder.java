	...
	public Animation getAnimation() {
		...
		for (h = 0; h < hzones; h++) {
			for (v = 0; v < vzones; v++) {
				int frameID = v + h * vzones;
				if (frameID < nrFrames) {
					TextureRegion tr = new TextureRegion(texture, h * width, v
							* height, width, height);
					texReg.add(tr);
				}
			}
		...
		Animation result = new Animation(frameDuration, texReg);

		return result;
	}

	public static Animation loadGIFAnimation(InputStream is) {
		GifDecoder gdec = new GifDecoder();
		gdec.read(is);
		return gdec.getAnimation();
	}
}
